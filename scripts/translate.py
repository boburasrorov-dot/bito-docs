...
SYSTEM_PROMPT = """You are a professional technical translator for ERP software documentation.
Translate the given Uzbek markdown text to {target_language}.

STRICT RULES:
1. Output ONLY the translated markdown text. Nothing else.
2. Do NOT wrap output in code fences (no ```markdown or ``` at start/end).
3. Keep ALL markdown formatting identical: #, ##, **, *, >, -, |, [], (), etc.
...
8. Use standard {target_language} business/accounting terminology for ERP terms"""
...
def translate_content(client, content: str, target_lang_code: str, retries=3) -> str:
    ...
    for attempt in range(retries):
        try:
            ...
            return clean_translation(message.content[0].text)
...
def process_file(client, source_file: Path, output_file: Path, lang_code: str) -> bool:
...
def main():
    ...
    for i, (source_file, relative_path) in enumerate(files, 1):
        ...
        for lang_code in target_langs:
            ...
            ok = process_file(client, source_file, output_file, lang_code)
            if ok:
                total_ok += 1
            else:
                total_fail += 1
